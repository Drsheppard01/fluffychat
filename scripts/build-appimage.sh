...
URUNTIME_URL="https://github.com/VHSgunzo/uruntime/releases/latest/download/uruntime-appimage-squashfs-$ARCH"

# Prepare AppDir
cp -r build/linux/*/release/bundle/. "$BUILD_DIR"/AppDir
cp appimage/FluffyChat.desktop "$BUILD_DIR"/AppDir
cp assets/logo.svg "$BUILD_DIR"/AppDir/fluffychat.png

# Add libraries
wget "$LIB4BN_URL" -O "$BUILD_DIR"/lib4bin
chmod +x "$BUILD_DIR"/lib4bin
xvfb-run -a -- "$BUILD_DIR"/lib4bin l -s -k -e -v -p -d "$BUILD_DIR"/AppDir "$BUILD_DIR"/AppDir/fluffychat /usr/lib/*/libGL*
...
"$BUILD_DIR"/uruntime --appimage-mkdwarfs -f \
	...
	-i "$BUILD_DIR"/AppDir -o dist/fluffychat-linux-$ARCH.AppImage
